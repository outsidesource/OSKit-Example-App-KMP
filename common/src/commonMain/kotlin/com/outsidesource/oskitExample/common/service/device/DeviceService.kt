...
class DeviceService {
    ...
    suspend fun getVolume(id: Int): Outcome<Float> {
        randomDelay(50, 150)
        return Outcome.Ok(volumes[id] ?: 0f)
    }

    suspend fun getMode(id: Int): Outcome<DeviceMode> {
        randomDelay(50, 150)
        return Outcome.Ok(modes[id] ?: DeviceMode.Listen)
    }

    suspend fun connect(id: Int): Outcome<Unit> {
        delay(750)
        connectionStatuses[id] = ConnectionStatus.Connected
        return Outcome.Ok(Unit)
    }

    fun disconnect(id: Int): Outcome<Unit> {
